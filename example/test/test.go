...
func main() {
	...
	// 创建节点
	node, err := goch.NewServer(addr, "go-cache",
		goch.WithEtcdEndpoints([]string{"localhost:2379"}),
		goch.WithDialTimeout(5*time.Second),
	)
	if err != nil {
		log.Fatalln("创建节点失败: ", err)
	}
	// 创建节点选择器
	picker, err := goch.NewClientPicker(addr)
	if err != nil {
		log.Fatalln("创建节点选择器失败: ", err)
	}
	// 创建缓存组
	group := goch.NewGroup("test", 2<<20, goch.GetterFunc(
	...
	go func() {
		...
		if err := node.Start(); err != nil {
			log.Fatalln("启动节点失败: ", err)
		}
	}()
	// 等待节点注册wanc
	log.Printf("[节点%s] 等待节点注册...\n", *nodeID)
	...
	fmt.Printf("\n=== 节点%s: 设置本地数据 ===\n", *nodeID)
	err = group.Put(ctx, localKey, localValue)
	if err != nil {
		log.Fatalln("设置本地数据失败: ", err)
	}
